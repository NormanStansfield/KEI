...
// component
export class BatchModeComponent implements Mithril.Component<BatchModeController> {
  ...
  onclickOK = async () => {
    ...
    const promiseAry = this.vm.cwdFiles().filter((elm) => {return elm.checked()}).map( async (fsstats: ImgFileInfo) => {
  ...
  viewBatchMode = (ctrl) => {
    return m(".window-content",
      m(".pane-group",
        [
          m(".pane", {style: {"width": "100%", "max-width": "520px"}},
            m(".window",
              m(".window-content",
                m(".padded-more",
                  [
                    m(".form-group",
                      [
                        ...
                        m("input.form-control[autofocus=''][placeholder='Title'][size='100'][type='text']", {onchange: m.withAttr("value", this.title), value: this.title()})
                      ]
                    ),
                    m(".form-group",
                      [
                        ...
                        m("input.form-control[placeholder='keywords with separator'][size='100'][type='text']", {onchange: m.withAttr("value", this.keywords), value: this.keywords()})
                      ]
                    ),
                    m(".form-action.pull-right",
                      [
                        ...
                        m.trust("&emsp;"),
                        m("span[class='']", {onclick: this.onclickOK},
                          m("button.btn.btn-large.btn-default",
                            [
                ...
                m("tbody",
                  [
                    this.vm.cwdFiles().filter((elm) => {return elm.checked()}).map((element: ImgFileInfo) => {
                      return m("tr",
                        [
                          m("td", {onclick: () => {element.checked(!element.checked())}},
