...
export class MainWComponent implements Mithril.Component<MainWController> {
  ...
  onclickFilename = (filename) => {
    ...
    let previeWin = new BrowserWindow ({ width: width, height: height, frame: false }) // frameless window
                ...
                this.vm.cwdFiles().filter((elm) => {return elm.checked()}).map((element: ImgFileInfo) => {
                  return m("p.nav-group-item",
                    [
                      ...
                      m("br"),
                      m("strong",
                        element.iptc().Keywords ? new Array(element.iptc().Keywords).join() : 'Empty'
                      ),
                      m("hr")
                    ]
                  )
                    ...
                    this.vm.cwdFiles().map ((element: ImgFileInfo) => {
                      return m("tr",
                        [
                          ...
                          m("td", {onclick: m.withAttr("textContent", (value) => {console.log (value); this.onclickFilename(value)})},
                            element.file().name
                          ),
                          m("td",
                            element.file().mtime
