...
export class ToolbarComponent implements Mithril.Component<ToolbarController> {
        ...
        await this.getIPTC(element)
          .then((res) => {
            if (typeof res === 'string') {
              // if response is string, get error message in getIPTC
  ...
  onclickFolder = () => {
    ...
    // let folderName = dialog.showOpenDialog(null, {
    let folderName: string[] = dialog.showOpenDialog({
      properties: ['openDirectory'],
      title: 'Select Folder',
      defaultPath: currentDir(),
    })
    ;(() => {
      if (!(typeof folderName === 'undefined' || folderName.length === 0)) {
        // if choose folder
  ...
  onclickAbout = () => {
    ...
    aboutWin.show()
    aboutWin.on('blur', () => {
      console.log('aboutWin', aboutWin.id)
  ...
  // view
  viewToolbar = (ctrl) => {
    return m('header.toolbar.toolbar-header#toolbar-header', [
      m('h1.title', ''),
      m('.toolbar-actions', [
        m('.btn-group', [
          m(
            'button.btn.btn-default.btn-large#home',
            {onclick: this.onclickHome},
            [m('', [m('span.icon.icon-home'), m.trust('&thinsp;'), 'Home'])]
          ),

          m(
            'button.btn.btn-default.btn-large#folder',
            {onclick: this.onclickFolder},
            [m('', [m('span.icon.icon-folder'), m.trust('&thinsp;'), 'Folder'])]
          ),
        ]),
        m(
          'button.btn.btn-default.btn-large#refresh',
        ...
        m(
          ...
          [m('', [m('span.icon.icon-brush'), m.trust('&thinsp;'), 'Set Tags'])]
        ),
        m('.btn-group.pull-right', [
          m(
            'button.btn.btn-default.btn-large#settings',
            {
              class: this.settingmode() ? 'btn-primary' : '',
              onclick: this.onclickSettings,
            },
            [m('', [m('span.icon.icon-cog'), m.trust('&thinsp;'), 'Settings'])]
          ),
          m(
            'button.btn.btn-default.btn-large#about',
